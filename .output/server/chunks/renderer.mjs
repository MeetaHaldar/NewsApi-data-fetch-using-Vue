...
(function (module) {
	...
	{
	  module.exports = shared_cjs_prod;
	}
} (shared$2));
...
function triggerEffects(dep, debuggerEventExtraInfo) {
    ...
    for (const effect of shared$1.isArray(dep) ? dep : [...dep]) {
...
function createGetter(isReadonly = false, shallow = false) {
    return function get(target, key, receiver) {
        ...
        if (isRef(res)) {
            ...
            const shouldUnwrap = !targetIsArray || !shared$1.isIntegerKey(key);
...
(function (module) {
	...
	{
	  module.exports = reactivity_cjs_prod;
	}
} (reactivity));
	...
	*/
	...
	function baseCreateRenderer(options, createHydrationFns) {
	    ...
	    const setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, isSVG, optimized) => {
	        ...
	        const effect = (instance.effect = new reactivity$1.ReactiveEffect(componentUpdateFn, () => queueJob(instance.update), instance.scope // track it in component's effect scope
	        ));
...
} (runtimeCore_cjs_prod));

(function (module) {

	{
	  module.exports = runtimeCore_cjs_prod;
	}
} (runtimeCore));
...
(function (module) {
	...
	{
	  module.exports = runtimeDom_cjs_prod;
	}
} (runtimeDom));
...
function renderToNodeStream(input, context = {}) {
    const stream = new (require$$3.Readable)()
        ;
    if (!stream) {
